...
from business_app.utils.decorators import cache_response
...
def get_blog_posts():
    ...
    try:
        ...
        query = BlogPost.query.filter(
            BlogPost.status == BlogStatus.PUBLISHED, BlogPost.published_at <= datetime.now(UTC)
        )
        ...
        query = query.order_by(desc(BlogPost.is_featured), desc(BlogPost.published_at))
...
def get_blog_post(slug):
    ...
    try:
        ...
        return success_response(data=post.to_dict(language), message="Blog post retrieved successfully")
...
def get_featured_posts():
    ...
    try:
        ...
        return success_response(data=posts_data, message=f"{len(posts_data)} featured posts retrieved successfully")
