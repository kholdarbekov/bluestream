...
from business_app.utils.service_factory import get_product_service, get_review_service
...
def get_categories():
    ...
    try:
        ...
        return success_response(data={"categories": [serialize_product_category(cat, language) for cat in categories]})
...
def get_category(category_id):
    ...
    try:
        ...
        return success_response(data={"category": serialize_product_category(category, language)})
...
def get_products():
    ...
    try:
        ...
        return paginated_response(
            items=[serialize_product(product, language, current_user) for product in products],
            page=page,
            per_page=per_page,
            total=total,
...
def get_product(product_id):
    ...
    try:
        ...
        product = product_service.get_product_by_id(
            product_id=product_id, current_user_id=current_user.id if current_user else None
        )
        ...
        reviews, _, _, _ = review_service.get_product_reviews(
            product_id=product_id, page=1, per_page=5, sort_by="recent", approved_only=True
        )

        return success_response(
            data={
                "product": serialize_product(product, language, current_user, quantity),
                # 'related_products': [
                #     serialize_product(p, language, current_user)
                #     for p in related_products
                # ],
                "reviews": [ReviewSerializer(review).to_dict() for review in reviews],
            }
        )
...
def get_product_reviews(product_id):
    ...
    try:
        ...
        sort_by_mapping = {"created_at": "recent", "rating": "highest", "helpful": "helpful"}
        ...
        reviews, total, page, per_page = review_service.get_product_reviews(
            product_id=product_id, page=page, per_page=per_page, sort_by=sort_by, approved_only=True
        )
        ...
        return paginated_response(
            items=[ReviewSerializer(review).to_dict() for review in reviews],
            page=page,
            per_page=per_page,
            total=total,
            additional_meta={"summary": review_stats},
        )
...
def add_product_review(product_id):
    ...
    try:
        ...
        has_purchased = (
            ...
            .join(Order)
            .filter(Order.user_id == current_user_id, OrderItem.product_id == product_id, Order.status == "delivered")
            .first()
        )
        ...
        return created_response(
            data={"review": ReviewSerializer(review).to_dict()}, message=get_translation("success.saved")
        )
...
def get_featured_products():
    ...
    try:
        ...
        product_service = get_product_service()
        products = product_service.get_featured_products(limit=limit, language=language)

        return success_response(
            data={"featured_products": [serialize_product(product, language, current_user) for product in products]}
        )
...
def get_products_bulk():
    ...
    try:
        ...
        if not data or "product_ids" not in data:
            return validation_error_response(errors={"product_ids": "Product IDs are required"})

        product_ids = data.get("product_ids", [])

        if not isinstance(product_ids, list):
            return validation_error_response(errors={"product_ids": "Product IDs must be a list"})

        if len(product_ids) == 0:
            return success_response(data={"products": []})

        if len(product_ids) > 50:
            return validation_error_response(errors={"product_ids": "Maximum 50 products can be fetched at once"})
        ...
        return success_response(
            data={"products": result, "found_count": len(result), "requested_count": len(product_ids)}
        )
...
def get_search_suggestions():
    ...
    try:
        ...
        product_service = get_product_service()
        suggestions = product_service.get_search_suggestions(query=query, limit=limit, language=language)
...
def get_popular_products():
    ...
    try:
        ...
        popular_rows = product_service.get_popular_products(
            period_days=period_to_days.get(period, 7), limit=limit, language=language
        )
        ...
        return success_response(
            data={
                "popular_products": [
                    {
                        "product": serialize_product(row["product"], language, current_user),
...
def calculate_price():
    ...
    try:
        ...
        pricing_data = product_service.calculate_product_price(
            product_id=product_id, quantity=quantity, user=current_user, promo_code=data.get("promo_code")
        )
