...
def get_user_sessions():
    ...
    try:
        ...
        sessions_query = UserSession.query.filter_by(user_id=user_id).order_by(
...
def get_all_sessions():
    ...
    try:
        ...
        # Build query
        sessions_query = UserSession.query.join(User).order_by(
            UserSession.last_activity.desc()
        )
...
def trigger_cleanup():
    ...
    try:
        ...
        return jsonify({
            'message': f'{action.title()} cleanup completed',
            'results': results
        }), 200
...
def get_session_stats():
    ...
    try:
        ...
        return jsonify({
            'statistics': stats,
            'timestamp': UserSession.query.first().created_at.isoformat() if UserSession.query.first() else None
        }), 200
