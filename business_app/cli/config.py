...
from business_app.config import get_config, validate_environment, get_environment_info, config
...
def info(detailed):
    ...
    if detailed:
        ...
        click.echo(
            f"  Database URI: {env_info['database_uri'][:50]}..."
            if len(env_info["database_uri"]) > 50
            else f"  Database URI: {env_info['database_uri']}"
        )
        click.echo(
            f"  Redis URL: {env_info['redis_url'][:50]}..."
            if len(env_info["redis_url"]) > 50
            else f"  Redis URL: {env_info['redis_url']}"
        )
...
def security_check(check_secrets):
    ...
    if check_secrets:
        placeholder_patterns = ["REPLACE_WITH_", "your_", "change_me", "secret-key-change", "dev-secret-key"]
