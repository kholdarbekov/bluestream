...
if use_fallback:
    ...
    def get_secret(secret_name: str, env_var: str = None, default: str = None, required: bool = True):  # noqa: F811
    ...
    def get_database_url():  # noqa: F811
...
class BaseConfig:
    ...
    # ARCH-010: exact placeholders that must never reach production.
    ...
    # Substrings that are nearly always a sign of a placeholder, even if padded.
    ...
    @classmethod
    def _secret_is_weak(cls, value: str) -> bool:
        """Heuristic: reject known placeholders and low-entropy values."""
        lowered = value.strip().lower()
        if lowered in cls._WEAK_SECRET_VALUES:
            return True
        if any(marker in lowered for marker in cls._WEAK_SECRET_SUBSTRINGS):
            return True
        # A secret that is all one character, or a trivial repeating pattern, is weak.
    ...
    def validate_secret_key(cls):
        ...
        if cls._secret_is_weak(secret_key):
            raise ValueError("SECRET_KEY looks like a placeholder or low-entropy value; generate a fresh random secret")
    ...
    PAYME_MERCHANT_ID_WITH_BILLING = get_secret(
        "payme_merchant_id_with_billing", "PAYME_MERCHANT_ID_WITH_BILLING", required=False
    )
    ...
    PAYME_SECRET_KEY_WITH_BILLING = get_secret(
        "payme_secret_key_with_billing", "PAYME_SECRET_KEY_WITH_BILLING", required=False
    )
    ...
    CLICK_SHOP_MERCHANT_ID = (
        get_secret("click_shop_merchant_id", "CLICK_SHOP_MERCHANT_ID", required=False) or CLICK_MERCHANT_ID
    )
    CLICK_SHOP_SERVICE_ID = (
        get_secret("click_shop_service_id", "CLICK_SHOP_SERVICE_ID", required=False) or CLICK_SERVICE_ID
    )
    CLICK_SHOP_SECRET_KEY = (
        get_secret("click_shop_secret_key", "CLICK_SHOP_SECRET_KEY", required=False) or CLICK_SECRET_KEY
    )
    ...
    CLICK_MERCHANT_API_URL = os.environ.get(
        "CLICK_MERCHANT_API_URL", os.environ.get("CLICK_ENDPOINT_URL", "https://api.click.uz/v2/merchant")
    )
    ...
    CLICK_MERCHANT_API_PASSWORD = get_secret(
        "click_merchant_api_password", "CLICK_MERCHANT_API_PASSWORD", required=False
    )
    CLICK_MERCHANT_API_SECRET_KEY = get_secret(
        "click_merchant_api_secret_key", "CLICK_MERCHANT_API_SECRET_KEY", required=False
    )
    CLICK_MERCHANT_API_SECRET = (
    ...
    CLICK_CALLBACK_ALLOWLIST = [
        ip.strip() for ip in os.environ.get("CLICK_CALLBACK_ALLOWLIST", "").split(",") if ip.strip()
    ]
...
For complete documentation, examples, and SDKs visit: [API Documentation](/docs/api)
        ...
        "externalDocs": {"description": "Complete API Documentation", "url": "/docs/api"},
    }
    ...
    LANGUAGES = {"en": "English", "uz": "O'zbek", "ru": "Русский"}
    DEFAULT_LANGUAGE = "uz"
