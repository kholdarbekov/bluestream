...
from flask import render_template, request, session, current_app, redirect, url_for, flash, g, Response
...
def _render_product_detail_page(product, language):
    ...
    related_products = (
        Product.query.filter(
            Product.category_id == product.category_id, Product.id != product.id, Product.is_active == True
        )
        .limit(4)
        .all()
    )
...
def index():
    ...
    try:
        ...
        current_app.logger.info(
            f"[INDEX-DEBUG] [REQ:{request_id}] Fetched {len(featured_products)} featured products with language='{language}'"  # noqa: E501
        )
    ...
    try:
        ...
        current_app.logger.info(
            f"[INDEX-DEBUG] [REQ:{request_id}] Fetched {len(categories)} categories with language='{language}'"
        )
    except Exception as e:
    ...
    try:
        featured_posts = (
            ...
            .order_by(desc(BlogPost.sort_order), desc(BlogPost.published_at))
            .limit(3)
            .all()
        )
    except Exception as e:
    ...
    try:
        ...
        if current_user_id:
            ...
            if user:
                user_data = {"id": user.id, "name": f"{user.first_name} {user.last_name}", "email": user.email}
...
def shop():
    ...
    # Paginate products
    products_pagination = query.order_by(Product.created_at.desc()).paginate(page=page, per_page=12, error_out=False)
...
def checkout():
    ...
    return render_template("frontend/checkout.html", user=user, addresses=addresses, today=today)
...
def payment_success():
    ...
    if order_id:
        ...
        if order:
            ...
            payment = Payment.query.filter_by(order_id=order.id).order_by(desc(Payment.created_at)).first()
            ...
            if payment and payment.status.value == "pending":
                return render_template("frontend/payment_pending.html", order=order, payment=payment)
...
def my_subscriptions():
    ...
    subscriptions = Subscription.query.filter_by(user_id=current_user_id).order_by(desc(Subscription.created_at)).all()
    ...
    try:
        ...
        overview_data = {"active_count": len(active_subscriptions), "deliveries_this_month": 0, "monthly_savings": 0}

    return render_template(
        "frontend/my_subscriptions.html",
        user=user,
...
def my_orders():
    ...
    order_statuses = [
        {"value": status.value, "label": status.value.replace("_", " ").title()} for status in OrderStatus
    ]
...
def set_language_route(language):
    ...
    domains = [None, ".aqua-element.uz", "aqua-element.uz", ".localhost", "localhost"]  # Host only
...
def sitemap_index():
    ...
    try:
        product_lastmod_raw = db.session.query(func.max(func.coalesce(Product.updated_at, Product.created_at))).scalar()
        blog_lastmod_raw = (
            db.session.query(func.max(BlogPost.published_at))
            .filter(BlogPost.status == BlogStatus.PUBLISHED, BlogPost.published_at <= datetime.now(UTC))
            .scalar()
        )
    except Exception:
        ...
        entries.append(
            {
                ...
                "lastmod": _format_lastmod(
                    getattr(product, "updated_at", None) or getattr(product, "created_at", None)
                ),
                "changefreq": "weekly",
                "priority": "0.7",
            }
...
def sitemap_blog():
    ...
    posts = (
        BlogPost.query.filter(BlogPost.status == BlogStatus.PUBLISHED, BlogPost.published_at <= datetime.now(UTC))
...
def google_products_feed():
    ...
    for product in products:
        ...
        image_link = (
            _as_absolute_url(images[0]) if images else _as_absolute_url(url_for("static", filename="images/logo.png"))
        )
        additional_images = []
...
def blog_list():
    ...
    query = BlogPost.query.filter(BlogPost.status == BlogStatus.PUBLISHED, BlogPost.published_at <= datetime.now(UTC))
...
def blog_detail(slug):
    ...
    post = BlogPost.query.filter(
        BlogPost.slug == slug, BlogPost.status == BlogStatus.PUBLISHED, BlogPost.published_at <= datetime.now(UTC)
    ).first_or_404()
    ...
    if normalized_tags:
        ...
        if tag_conditions:
            recommended_products = (
                Product.query.filter(Product.is_active == True, or_(*tag_conditions))
    ...
    if len(recommended_products) < 4:
        ...
        fallback_products = (
            fallback_query.order_by(desc(Product.is_featured), desc(Product.created_at))
            .limit(4 - len(recommended_products))
            .all()
        )
