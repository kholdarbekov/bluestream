    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            try:
                ...
                return (
                    jsonify({"success": False, "message": "Authentication required", "error_code": "AUTH_REQUIRED"}),
                    401,
                )
        ...
        def decorated_function(*args, **kwargs):
            try:
                ...
                if user.status != UserStatus.ACTIVE.value:
                    return (
                        jsonify(
                            {"success": False, "message": "Account is not active", "error_code": "ACCOUNT_INACTIVE"}
                        ),
                        403,
                    )
                ...
                return jsonify({"success": False, "message": "Authorization failed", "error_code": "AUTH_FAILED"}), 401
...
def staff_required(f):
    ...
    """
    return require_role([UserRole.ADMIN.value, UserRole.MANAGER.value, UserRole.OPERATOR.value])(f)
...
def manager_or_admin_required(f):
    ...
    """
    return require_role([UserRole.ADMIN.value, UserRole.MANAGER.value])(f)
...
def customer_or_staff_required(f):
    ...
    """
    return require_role(
        [UserRole.CUSTOMER.value, UserRole.ADMIN.value, UserRole.MANAGER.value, UserRole.OPERATOR.value]
    )(f)
    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            try:
                ...
                return (
                    jsonify({"success": False, "message": "Rate limiting failed", "error_code": "RATE_LIMIT_ERROR"}),
                    500,
                )
            ...
            try:
                ...
                if jti:
                    ...
                    if token_service.is_token_blacklisted(jti):
                        ...
                        return (
                            jsonify(
                                {"success": False, "message": "Token has been revoked", "error_code": "TOKEN_REVOKED"}
                            ),
                            401,
                        )
                ...
                return (
                    jsonify({"success": False, "message": "Token validation failed", "error_code": "TOKEN_INVALID"}),
                    401,
                )
    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            try:
                ...
                if not user_id:
                    return (
                        jsonify({"success": False, "message": "Invalid session", "error_code": "INVALID_SESSION"}),
                        401,
                    )
