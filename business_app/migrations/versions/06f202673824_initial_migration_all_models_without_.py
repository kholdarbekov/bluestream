...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column(
            "role",
            sa.Enum("customer", "admin", "manager", "delivery_driver", "operator", name="user_role"),
            nullable=True,
        ),
        sa.Column(
            "status",
            sa.Enum("active", "inactive", "banned", "pending_verification", "merged", name="user_status"),
            nullable=True,
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "channel",
            sa.Enum("email", "sms", "telegram", "push", "in_app", name="notification_channel"),
            nullable=False,
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "status",
            sa.Enum("active", "paused", "cancelled", "expired", "trial", name="subscription_status"),
            nullable=True,
        ),
        sa.Column("name", sa.String(length=200), nullable=False),
        sa.Column("description", sa.Text(), nullable=True),
        ...
        sa.Column(
            "delivery_frequency",
            sa.Enum("daily", "weekly", "biweekly", "monthly", name="subscription_frequency"),
            nullable=False,
        ),
        ...
        sa.Column(
            "payment_method",
            sa.Enum("cash", "card", "payme", "click", "loyalty_points", "business_account", name="payment_method"),
            nullable=False,
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "transaction_type",
            sa.Enum("earned", "redeemed", "expired", "bonus", "adjustment", name="loyalty_transaction_type"),
            nullable=False,
        ),
    ...
    with op.batch_alter_table("payments", schema=None) as batch_op:
        ...
        batch_op.create_index(
            batch_op.f("ix_payments_provider_transaction_id"), ["provider_transaction_id"], unique=False
        )
    ...
    op.create_table(
        ...
        sa.Column(
            "delivery_status",
            sa.Enum("pending", "sent", "delivered", "failed", "read", name="notification_status"),
            nullable=True,
        ),
    ...
    with op.batch_alter_table("payment_transactions", schema=None) as batch_op:
        ...
        batch_op.create_index(
            batch_op.f("ix_payment_transactions_provider_transaction_id"), ["provider_transaction_id"], unique=False
        )
...
def downgrade():
    ...
    # ARCH-013 / TST-004: Postgres ENUM types are schema-level objects that
    # outlive their owning tables. ``op.drop_table()`` does not drop them, so
    # without this cleanup a subsequent ``upgrade()`` re-runs ``CREATE TYPE``
    # and fails with ``DuplicateObject``. ``IF EXISTS`` keeps the downgrade
    # idempotent against partial earlier failures.
    for enum_name in (
        "auditeventtype",
        "auditseverity",
        "blog_category",
