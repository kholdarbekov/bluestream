...
def upgrade():
    ...
    with op.batch_alter_table('driver_cash_transfers', schema=None) as batch_op:
        batch_op.create_index('idx_driver_cash_transfers_session_created', ['driver_cash_session_id', 'created_at'], unique=False)
        batch_op.create_index('idx_driver_cash_transfers_status_created', ['transfer_status', 'created_at'], unique=False)
        batch_op.create_index(batch_op.f('ix_driver_cash_transfers_checkpoint_confirmed_by_user_id'), ['checkpoint_confirmed_by_user_id'], unique=False)
        batch_op.create_index(batch_op.f('ix_driver_cash_transfers_driver_cash_session_id'), ['driver_cash_session_id'], unique=False)
        batch_op.create_index(batch_op.f('ix_driver_cash_transfers_driver_confirmed_by_user_id'), ['driver_confirmed_by_user_id'], unique=False)
    ...
    with op.batch_alter_table('driver_cash_sessions', schema=None) as batch_op:
        batch_op.add_column(sa.Column('gross_cash_collected', sa.Numeric(precision=12, scale=2), nullable=False, server_default='0'))
        batch_op.add_column(sa.Column('transferred_cash_total', sa.Numeric(precision=12, scale=2), nullable=False, server_default='0'))
        batch_op.add_column(sa.Column('expected_cash_on_hand', sa.Numeric(precision=12, scale=2), nullable=False, server_default='0'))
