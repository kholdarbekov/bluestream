...
def upgrade():
    ...
    with op.batch_alter_table('credit_cards', schema=None) as batch_op:
        batch_op.add_column(
            sa.Column('verification_attempts', sa.Integer(), nullable=True, default=0)
        )
        batch_op.add_column(
        ...
        batch_op.add_column(
            sa.Column('payme_recurrent', sa.Boolean(), nullable=True, default=False)
        )
    ...
    with op.batch_alter_table('credit_cards', schema=None) as batch_op:
        batch_op.alter_column('verification_attempts',
                              existing_type=sa.Integer(),
                              nullable=False,
                              server_default='0')
        batch_op.alter_column('payme_recurrent',
