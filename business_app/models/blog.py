...
class BlogPost(db.Model, TimestampMixin, TranslatableMixin):
    ...
    # Categorization
    category = Column(
        SQLEnum(BlogCategory, name="blog_category", values_callable=lambda x: [e.value for e in x]),
        nullable=False,
        default=BlogCategory.HEALTH_TIPS,
    )
    ...
    # Publishing
    status = Column(
        SQLEnum(BlogStatus, name="blog_status", values_callable=lambda x: [e.value for e in x]),
        nullable=False,
    ...
    def get_category_display(self, language=None):
        ...
        category_translations = {
            "health_tips": {"en": "Health Tips", "ru": "Советы по здоровью", "uz": "Sog'liq maslahatlari"},
            "water_benefits": {"en": "Water Benefits", "ru": "Польза воды", "uz": "Suv foydalari"},
            "company_news": {"en": "Company News", "ru": "Новости компании", "uz": "Kompaniya yangiliklari"},
            "quality_assurance": {"en": "Quality Assurance", "ru": "Контроль качества", "uz": "Sifat nazorati"},
            "lifestyle": {"en": "Lifestyle", "ru": "Образ жизни", "uz": "Turmush tarzi"},
            "environment": {"en": "Environment", "ru": "Экология", "uz": "Atrof-muhit"},
        }
    ...
    def to_summary_dict(self, language=None):
        ...
        result = {
            ...
            "published_at": self.published_at.isoformat() if self.published_at else None,
            "view_count": self.view_count,
            "author": {"name": self.author_name or (self.author.full_name if self.author else "Admin")},
