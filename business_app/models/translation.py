...
class Translation(db.Model):
    ...
    def get_translation(cls, key: str, language: str = "en"):
        ...
        return cls.query.filter_by(key=key, language=language, is_active=True).first()

    @classmethod
    def get_translations_by_category(cls, category: str, language: str = "en"):
        """Get all translations in a category"""
        return cls.query.filter_by(category=category, language=language, is_active=True).all()

    @classmethod
    def get_all_translations(cls, language: str = "en"):
        """Get all translations for a language"""
        return cls.query.filter_by(language=language, is_active=True).all()
    ...
    @classmethod
    def set_entity_translation(
        cls, entity_type: str, entity_id: int, field_name: str, language: str, value: str, user_id: int = None
    ):
    ...
    @classmethod
    def bulk_set_entity_translations(
        cls, entity_type: str, entity_id: int, translations_dict: dict, user_id: int = None
    ):
        """
        ...
        for field_name, translations in translations_dict.items():
            for language, content in translations.items():
                if content:  # Only set non-empty content
                    cls.set_entity_translation(entity_type, entity_id, field_name, language, content, user_id)
