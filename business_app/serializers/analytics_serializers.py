...
def generate_sales_analytics(data: Dict[str, Any], period: str) -> Dict[str, Any]:
    ...
    if "revenue_by_category" in data:
        for category_data in data["revenue_by_category"]:
            chart_point = ChartDataPointSchema(
                label=category_data["category"], value=category_data["revenue"], category=category_data["category"]
            )
    ...
    if "revenue_trend" in data:
        for trend_point in data["revenue_trend"]:
            chart_point = ChartDataPointSchema(
                label=trend_point["date"], value=trend_point["revenue"], date=trend_point["date"]
            )
    ...
    if "orders_trend" in data:
        for trend_point in data["orders_trend"]:
            chart_point = ChartDataPointSchema(
                label=trend_point["date"], value=trend_point["orders"], date=trend_point["date"]
            )
...
def calculate_conversion_funnel(funnel_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for i, stage in enumerate(stages):
        ...
        processed_stages.append(
            {"stage_name": stage["name"], "count": stage["count"], "conversion_rate": round(conversion_rate, 2)}
        )
...
def serialize_user_segment(segment) -> Dict[str, Any]:
    ...
    try:
        return {
            ...
            "created_at": (
                segment.created_at.isoformat() if hasattr(segment, "created_at") and segment.created_at else None
            ),
        }
    except Exception:
        return {
