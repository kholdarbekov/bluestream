...
def serialize_loyalty_reward(reward, user=None) -> Dict[str, Any]:
    ...
    try:
        ...
        data = {
            ...
            "reward_type": (
                reward.reward_type
                if isinstance(reward.reward_type, str)
                else (reward.reward_type.value if reward.reward_type else None)
            ),
            ...
            "tier_requirement": (
                reward.tier_requirement.value
                if hasattr(reward, "tier_requirement") and reward.tier_requirement
                else None
            ),
            "usage_limit_per_user": getattr(reward, "usage_limit_per_user", None)
            or getattr(reward, "max_uses_per_user", None),
            "total_usage_limit": getattr(reward, "total_usage_limit", None) or getattr(reward, "max_redemptions", None),
            "current_usage_count": getattr(reward, "current_usage_count", 0)
            or getattr(reward, "redemptions_used", 0)
            or 0,
        ...
        if user:
            ...
            if user_loyalty:
                ...
                data["can_redeem"] = (
                    reward.is_active
                    and current_points >= (reward.points_required or 0)
                    and (not reward.tier_requirement or user_tier == reward.tier_requirement.value)
                    and (not reward.total_usage_limit or reward.current_usage_count < reward.total_usage_limit)
                )
    ...
    try:
        ...
        return {
            ...
            "expires_at": (
                ...
                if hasattr(transaction, "expires_at") and transaction.expires_at
                else None
            ),
        }
    except Exception:
