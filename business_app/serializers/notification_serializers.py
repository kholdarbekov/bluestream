...
def serialize_bulk_notification(bulk_notification) -> Dict[str, Any]:
    ...
    try:
        data = {
            ...
            "notification_type": (
                bulk_notification.notification_type.value if bulk_notification.notification_type else None
            ),
        ...
        if bulk_notification.sent_count and bulk_notification.sent_count > 0:
            data["delivery_rate"] = round(
                ((bulk_notification.delivered_count or 0) / bulk_notification.sent_count) * 100, 1
            )
        else:
