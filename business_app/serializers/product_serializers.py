    ...
    try:
        ...
        product_data = {
            ...
            "pricing": {
                ...
                "discount_percentage": (
                    round(((base_price - current_price) / base_price * 100), 2) if base_price > 0 else 0
                ),
                "quantity": quantity,
...
def calculate_popularity_score(product) -> float:
    ...
    score = view_count * 0.1 + total_sold * 1.0 + rating * review_count * 0.5
...
def get_user_purchase_history(product, user) -> Dict[str, Any]:
    ...
    return {"total_purchased": 0, "last_purchase_date": None, "average_quantity": 0}
    ...
    if hasattr(product, "price_rules") and product.price_rules:
        for rule in product.price_rules:
            if (
                ...
                and (rule.max_quantity is None or quantity <= rule.max_quantity)
            ):
