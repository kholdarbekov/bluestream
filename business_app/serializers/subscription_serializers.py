...
class SubscriptionPreviewResponse(BaseModel):
    ...
    @field_validator('items_total', 'discount_amount', 'billing_amount', 'monthly_cost_estimate', 'regular_price_total', 'total_savings')
    @classmethod
    def validate_amounts(cls, v):
        return float(v)
...
def serialize_subscription(subscription, include_items=False, include_address=False) -> Dict[str, Any]:
    ...
    try:
        subscription_data = {
            ...
            'last_billing_date': subscription.last_billing_date.isoformat() if getattr(subscription, 'last_billing_date', None) else None,
            'next_billing_date': subscription.next_billing_date.isoformat() if getattr(subscription, 'next_billing_date', None) else None,
            'last_delivery_date': subscription.last_delivery_date.isoformat() if getattr(subscription, 'last_delivery_date', None) else None,
            'next_delivery_date': subscription.next_delivery_date.isoformat() if getattr(subscription, 'next_delivery_date', None) else None,
            'start_date': subscription.start_date.isoformat() if subscription.start_date else None,
            'end_date': subscription.end_date.isoformat() if getattr(subscription, 'end_date', None) else None,
            'pause_start_date': subscription.pause_start_date.isoformat() if getattr(subscription, 'pause_start_date', None) else None,
            'pause_end_date': subscription.pause_end_date.isoformat() if getattr(subscription, 'pause_end_date', None) else None,
