...
class CartService:
    ...
    def calculate_cart_estimate(
        ...
        if items_subtotal < self.min_order_amount:
            raise ValidationError(
                f"Minimum order amount is {self.min_order_amount} UZS. "
                f"Current cart total: {items_subtotal} UZS"
            )

        # Calculate potential loyalty points earned
        loyalty_points_earned = self._calculate_loyalty_points_earned(
            final_total,
            user
        )
    ...
    def validate_promo_code(
        ...
        if campaign.max_uses_per_customer:
            user_usage = db.session.query(func.count(Order.id)).filter(
        ...
        if campaign.min_order_value and cart_total < campaign.min_order_value:
            raise ValidationError(
                f"Minimum order value of {campaign.min_order_value} UZS required for this code"
            )
    ...
    def get_quick_reorder_suggestions(
        ...
        for item in frequent_items:
            ...
            if product:
                ...
                suggested_quantity = max(1, min(
                    int(item.total_quantity / item.order_count),
                    10  # Cap at 10
                ))
    ...
    def sync_cart_from_local(
        self,
        user_id: int,
        local_cart_items: List[Dict[str, Any]]
    ) -> Optional['Cart']:
        """
        ...
        for local_item in local_cart_items:
            ...
            if product_id in existing_items:
                ...
                existing_items[product_id].quantity = max(
                    existing_items[product_id].quantity,
                    quantity
                )
            else:
                # Add new item
    ...
    def _calculate_loyalty_points_earned(
        self,
        final_total: float,
        user: User
    ) -> int:
