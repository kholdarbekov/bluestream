...
from business_app.utils.helpers import sanitize_filename
...
class FileStorageService:
    ...
    def upload_image(
        ...
        try:
            ...
            self._validate_file(file, filename, "images")

            # Process image
            processed_image = self._process_image(file, resize, max_width, max_height, quality)

            # Upload original/processed image (skip validation - already validated above)
            file_info = self.upload_file(processed_image["main"], filename, folder, user_id, skip_validation=True)
            ...
            for size_name, thumbnail_data in processed_image.get("thumbnails", {}).items():
                ...
                thumb_info = self.upload_file(
                    thumbnail_data, thumb_filename, f"{folder}/thumbnails", user_id, skip_validation=True
                )
    ...
    def delete_file(self, file_path: str) -> bool:
        ...
        try:
            ...
            current_app.logger.exception("File deletion failed")
            return False
    ...
    def copy_file(self, source_path: str, dest_path: str) -> bool:
        ...
        try:
            ...
            current_app.logger.exception("File copy failed")
            return False
    ...
    def move_file(self, source_path: str, dest_path: str) -> bool:
        ...
        try:
            ...
            current_app.logger.exception("File move failed")
            return False
    ...
    def _validate_file(self, file: BinaryIO, filename: str, expected_category: str = None):
        ...
        try:
            ...
            current_app.logger.exception("File validation error")
            raise FileStorageError(f"File validation failed: {e}")
    ...
    def _process_image(
        self, file: BinaryIO, resize: bool, max_width: int, max_height: int, quality: int
    ) -> Dict[str, Any]:
    ...
    def _upload_to_local(self, file: BinaryIO, file_info: Dict[str, str], metadata: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _get_local_url(self, file_path: str) -> str:
        ...
        if has_request_context():
            ...
            current_app.logger.info(
                f"Request headers - Host: {request.host}, X-Forwarded-Host: {request.headers.get('X-Forwarded-Host')}, Referer: {request.headers.get('Referer')}"  # noqa: E501
            )
    ...
    # S3 storage methods
    def _upload_to_s3(self, file: BinaryIO, file_info: Dict[str, str], metadata: Dict[str, Any]) -> Dict[str, Any]:
        """Upload file to S3"""
        try:
            file.seek(0)
            ...
            self.s3_client.upload_fileobj(file, self.s3_bucket, file_info["file_path"], ExtraArgs=extra_args)
    ...
    def _get_s3_url(self, file_path: str, expires_in: int = 3600) -> str:
        ...
        try:
            return self.s3_client.generate_presigned_url(
                "get_object", Params={"Bucket": self.s3_bucket, "Key": file_path}, ExpiresIn=expires_in
            )
        except ClientError:
            return ""
    ...
    def _copy_s3_file(self, source_path: str, dest_path: str) -> bool:
        ...
        try:
            ...
            self.s3_client.copy_object(CopySource=copy_source, Bucket=self.s3_bucket, Key=dest_path)
            return True
        except ClientError:
            return False
