...
class InventoryService:
    ...
    def check_product_availability(self, product_id: int, requested_quantity: int, 
                                 exclude_order_id: Optional[int] = None) -> InventoryCheckResult:
        """
    ...
    def check_multiple_products_availability(self, 
                                           items: List[Dict[str, Any]], 
                                           exclude_order_id: Optional[int] = None) -> List[InventoryCheckResult]:
        """
    ...
    def reserve_inventory(self, order_id: int, items: List[Dict[str, Any]], 
                         user_id: Optional[int] = None, ttl: Optional[int] = None) -> Dict[str, Any]:
        """
        ...
        try:
            for item in items:
                ...
                reservation_key = f"inventory_reservation:{order_id}:{product_id}"
                reservation_data = {
                    'order_id': order_id,
                    'product_id': product_id,
                    'quantity': quantity,
                ...
                if redis_client:
                    ...
                    redis_client.hset(f"reservation_details:{order_id}:{product_id}", mapping=reservation_data)
                    redis_client.expire(f"reservation_details:{order_id}:{product_id}", ttl)
    ...
    def release_reservations(self, order_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            redis_client = self._get_redis_client()
            if redis_client:
                reservation_pattern = f"inventory_reservation:{order_id}:*"
                reservation_keys = redis_client.keys(reservation_pattern)
                
                details_pattern = f"reservation_details:{order_id}:*"
            ...
            if released_items:
                audit_logger.log_event(
                    ...
                    additional_data={
                        'order_id': order_id,
                        'released_products': released_items
                    }
                )
            ...
            return {
                'success': True,
                'released_items': len(released_items)
            }
    ...
    def _get_reserved_quantity(self, product_id: int, exclude_order_id: Optional[int] = None) -> int:
        ...
        try:
            pattern = f"inventory_reservation:*:{product_id}"
            ...
            logger.error(f"Failed to get reserved quantity for product {product_id}: {e}")
            return 0
                    ...
                    if details and 'expires_at' in details:
                        ...
                        if current_time > expires_at:
                            ...
                            if len(parts) >= 3:
                                ...
                                self.redis_client.delete(f"inventory_reservation:{order_id}:{product_id}")
                                cleaned_count += 1
                                
                except Exception as e:
                    logger.error(f"Error cleaning reservation key {key}: {e}")
                    continue
