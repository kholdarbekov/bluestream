...
class OrderService:
    ...
    def create_order(
        ...
        *,
        ...
        Args:
            ...
            subscription: When set, this order is generated by that subscription.
                The order is stamped with subscription_id / is_subscription_order
                inside the same transaction, and the subscription's
        ...
        # The MIN_ORDER_AMOUNT floor is checked against the GROSS basket. A
        # subscription discount is a merchant concession, not a smaller order —
        # gating on the net would hard-fail billing for any discounted
        ...
        if subscription is not None and subscription.discount_percentage:
            percentage = Decimal(str(subscription.discount_percentage))
        ...
        order = Order(
            ...
            # A subscription-generated order is an ordinary order that happens to
            # carry a subscription_id — nothing downstream may branch on origin.
    ...
    def repeat_order_for_user(self, order_id: int, user_id: int) -> Order:
        ...
        # The original order's payment method may be a retired provider
        # (payme) or, on legacy rows, NULL — either of which create_order now
        # rejects outright. Resolve to something repeatable up front so the
        # customer-facing Reorder button (offered on every DELIVERED order)
        # doesn't 400 on old orders. See shared/payment_methods.py for the
        # collapsing rules.
        try:
            repeat_payment_method = resolve_repeatable_payment_method(original_order.payment_method)
        except (UnknownPaymentMethodError, UnsupportedPaymentMethodError) as exc:
            raise ValidationError(
                get_translation("api.orders.error.repeat_legacy_order_unsupported"),
                error_code="REPEAT_LEGACY_ORDER_UNSUPPORTED",
            ) from exc
        ...
        order_data = {
            ...
            "payment_method": repeat_payment_method.value,
            "order_source": "web",
        }
    ...
    def _resolve_payment_method(
        ...
        *,
        ...
        bypass_cod_check: bool,
    ) -> PaymentMethod:
        """Resolve the order's payment method, or raise.

        Never returns None. A NULL ``orders.payment_method`` breaks the
        personal-card transfer validator, the driver's cash prompt, the COD
        debt cap and business-account settlement — all of which switch on it.

        When the caller supplies no method, a workplace entity whose lines are
        all contract-covered defaults to business-account settlement. Everyone
        else must state a method explicitly.
        """
        from business_app.services.corporate_contract_service import CorporateContractService
        ...
        if raw:
            try:
                ...
                raise ValidationError(f"Unknown payment method: {raw}") from exc
            try:
                assert_customer_selectable(payment_method)
            except UnsupportedPaymentMethodError as exc:
                raise ValidationError(str(exc)) from exc
        else:
            # Requirement: workplace-entity orders whose lines are ALL
            # contract-covered default to business-account settlement.
            try:
                CorporateContractService().validate_business_account_order(user=user, order_items=order_items)
            except ValidationError as exc:
                raise ValidationError("payment_method is required") from exc
            return PaymentMethod.BUSINESS_ACCOUNT
