...
class TaxCommitteeService:
    ...
    def _seed_token_from_config(self) -> TaxCommitteeApiToken:
        ...
        if not seed:
            raise ValidationError(
                "No Tax Committee API token available. "
                "Set TAX_COMMITTEE_API_TOKEN env var or seed via admin."
            )
    ...
    def refresh_token(self, old_token: str) -> str:
        ...
        if resp.status_code != 200:
            ...
            raise ValidationError(
                f"Failed to refresh Tax Committee API token: HTTP {resp.status_code}"
            )
    ...
    def check_marking_code_statuses(self, identification_codes: List[str]) -> Dict[str, str]:
        ...
        if resp.status_code != 200:
            ...
            raise ValidationError(
                f"Tax Committee status check failed: HTTP {resp.status_code} — {resp.text}"
            )
    ...
    def utilise_marking_codes(self, sntins: List[str], product: Product) -> Dict[str, Any]:
        ...
        if resp.status_code != 200:
            ...
            raise ValidationError(
                f"Tax Committee utilisation failed: HTTP {resp.status_code} — {resp.text}"
            )
