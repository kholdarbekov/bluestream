...
def update_real_time_metrics(activity_type: str, user_id: int = None):
    ...
    try:
        ...
        active_customers_today = db.session.query(Order.user_id).filter(
            ...
            for admin in admin_users:
                notification_service.send_notification(
                    ...
                    template_data={
                        ...
                        'total_at_risk': churn_predictions['high_risk_customers'] + churn_predictions['medium_risk_customers'],
...
def cleanup_old_analytics_data():
    ...
    try:
        ...
        deleted_behaviors = UserBehavior.query.filter(
            UserBehavior.timestamp < behavior_cutoff
        ).delete()
        
        # Delete old analytics reports (keep for 2 years)
        report_cutoff = datetime.now(timezone.utc) - timedelta(days=730)
        deleted_reports = AnalyticsReport.query.filter(
            AnalyticsReport.generated_at < report_cutoff
        ).delete()
        
        # Archive old sales metrics (keep for 3 years)
        metrics_cutoff = datetime.now(timezone.utc) - timedelta(days=1095)
        old_metrics = SalesMetric.query.filter(
            SalesMetric.date < metrics_cutoff.date()
        ).all()
...
def generate_business_insights(report_data: Dict[str, Any]) -> List[str]:
    ...
    try:
        ...
        if revenue_growth > 10:
            ...
            insights.append(f"Revenue declined by {abs(revenue_growth):.1f}% - investigate market factors and customer satisfaction")
        
        # Order insights
        completion_rate = report_data['overview']['orders'].get('completion_rate', 0)
        if completion_rate < 85:
            insights.append(f"Order completion rate of {completion_rate:.1f}% is below target - review fulfillment processes")
        ...
        if delivery_success_rate < 95:
            insights.append(f"Delivery success rate of {delivery_success_rate:.1f}% needs improvement - review delivery operations")
        
        avg_delivery_time = report_data['overview']['delivery'].get('average_delivery_time_hours', 0)
        if avg_delivery_time > 4:
            insights.append(f"Average delivery time of {avg_delivery_time:.1f} hours exceeds target - optimize routes and scheduling")
...
def monitor_business_kpis():
    ...
    try:
        ...
        yesterday_revenue = db.session.query(db.func.sum(Order.total_amount)).filter(
        ...
        total_deliveries_today = Delivery.query.filter(
            db.func.date(Delivery.created_at) == today
        ).count()
        ...
        if total_deliveries_today > 0:
            ...
            if failure_rate > 10:  # More than 10% failure rate
                alerts.append(f"High delivery failure rate: {failure_rate:.1f}% ({failed_deliveries_today}/{total_deliveries_today})")
...
def generate_product_performance_analysis():
    ...
    try:
        ...
        analysis = {
            ...
            'poor_performers': [p for p in product_performance if p['revenue'] < total_revenue * 0.01],  # Products with <1% of total revenue
            'recommendations': []
        }
        
        # Generate recommendations
        if analysis['poor_performers']:
            analysis['recommendations'].append(f"Consider discontinuing or promoting {len(analysis['poor_performers'])} underperforming products")
        ...
        if high_demand_products:
            analysis['recommendations'].append(f"Ensure adequate inventory for {len(high_demand_products)} high-demand products")
    ...
    try:
        ...
        for area in geographic_data:
            ...
            avg_delivery_time = db.session.query(
                db.func.avg(db.func.extract('epoch', Delivery.delivered_at - Delivery.created_at))
            ).join(Order).filter(
            ...
            delivery_success_rate = db.session.query(
        ...
        return {
            'success': True,
            'report_id': report.id,
            'areas_analyzed': len(area_metrics)
        }
