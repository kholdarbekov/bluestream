...
def cleanup_old_audit_logs_task(
    self, retention_days: int = 90, batch_size: int = 1000, preserve_critical: bool = True
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if preserve_critical:
            preserved_critical_count = AuditLog.query.filter(
                and_(AuditLog.created_at < cutoff_date, AuditLog.severity == AuditSeverity.CRITICAL)
            ).count()
        ...
        while True:
            ...
            batch_ids = [log.id for log in base_query.limit(batch_size).all()]

            if not batch_ids:
                break

            # Delete batch
            deleted_count = AuditLog.query.filter(AuditLog.id.in_(batch_ids)).delete(synchronize_session=False)
            db.session.commit()
...
def archive_old_audit_logs_task(
    self, retention_days: int = 90, batch_size: int = 1000, archive_format: str = "json"
) -> Dict[str, Any]:
    """
    ...
    try:
        logger.info(f"Starting audit log archive task " f"(retention: {retention_days} days, format: {archive_format})")
        ...
        logs_to_archive = AuditLog.query.filter(AuditLog.created_at < cutoff_date).order_by(AuditLog.created_at).all()

        total_archived = len(logs_to_archive)

        if total_archived == 0:
            logger.info("No audit logs to archive")
            return {"total_archived": 0, "archive_file": None, "status": "nothing_to_archive"}
...
def get_audit_log_statistics_task(self) -> Dict[str, Any]:
    ...
    try:
        ...
        if total_logs == 0:
            return {"total_logs": 0, "status": "no_logs"}
        ...
        logs_by_age = {
            "last_24h": AuditLog.query.filter(AuditLog.created_at >= now - timedelta(hours=24)).count(),
            "last_7_days": AuditLog.query.filter(AuditLog.created_at >= now - timedelta(days=7)).count(),
            "last_30_days": AuditLog.query.filter(AuditLog.created_at >= now - timedelta(days=30)).count(),
            "last_90_days": AuditLog.query.filter(AuditLog.created_at >= now - timedelta(days=90)).count(),
            "older_than_90_days": AuditLog.query.filter(AuditLog.created_at < now - timedelta(days=90)).count(),
        }
