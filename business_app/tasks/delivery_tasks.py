...
from business_app.utils.constants import DeliveryStatus, UserRole, OrderStatus
...
def auto_assign_delivery_task(self, delivery_id: int):
    ...
    try:
        ...
        if best_driver:
            # Assign delivery
            assigned_delivery = delivery_service.assign_delivery_driver(delivery_id, best_driver.id)
...
def generate_driver_performance_report(self, driver_id: int, start_date: str, end_date: str):
    ...
    try:
        ...
        deliveries = Delivery.query.filter(
            ...
            Delivery.created_at.between(start_dt, end_dt)
        ).all()
        ...
        if completed_deliveries:
            total_time = sum((d.actual_delivery_time - d.scheduled_date).total_seconds() for d in completed_deliveries if d.actual_delivery_time)
                    ...
                    for admin in admin_users:
                        notification_service.send_notification(
                            ...
                            template_data={
                                ...
                                'driver_name': f"{delivery.delivery_person.first_name} {delivery.delivery_person.last_name}" if delivery.delivery_person else 'Unassigned',
...
def update_delivery_zones_task(self):
    ...
    try:
        ...
        delivery_density = db.session.query(
            ...
            func.round(Delivery.order.delivery_latitude, 2),
            func.round(Delivery.order.delivery_longitude, 2)
        ).having(func.count(Delivery.id) >= 5).all()  # Minimum 5 deliveries to consider
...
def optimize_daily_delivery_routes():
    ...
    try:
        ...
        active_drivers = db.session.query(Delivery.delivery_person_id).filter(
            Delivery.status.in_([DeliveryStatus.ASSIGNED, DeliveryStatus.IN_TRANSIT, DeliveryStatus.PICKED_UP]),
        ...
        for delivery in upcoming_deliveries:
            try:
                ...
                customer_template_data = {
                    ...
                    'driver_name': f"{delivery.delivery_person.first_name} {delivery.delivery_person.last_name}" if delivery.delivery_person else None,
                ...
                notification_service.send_notification(
                    ...
                    'delivery_reminder',
                    template_data=customer_template_data
                )
                ...
                if delivery.delivery_person_id:
                    ...
                    notification_service.send_notification(
                        ...
                        'delivery_reminder',
                        template_data=driver_template_data
                    )
                    driver_reminders += 1
...
def calculate_delivery_eta_task(self, delivery_id: int):
    ...
    try:
        ...
        travel_time = maps_service.calculate_travel_time(
            delivery.current_location_lat, delivery.current_location_lng,
            delivery.order.delivery_latitude, delivery.order.delivery_longitude
        )
...
def process_delivery_analytics():
    ...
    try:
        ...
        zone_metrics = db.session.query(
            ...
            Delivery.created_at.between(start_date, end_date),
            Delivery.status == DeliveryStatus.DELIVERED
        ).group_by(Delivery.delivery_zone).all()
        ...
        # Driver performance
        driver_metrics = db.session.query(
            Delivery.driver_id,
            func.count(Delivery.id).label('delivery_count'),
            func.avg(func.extract('epoch', Delivery.delivered_at - Delivery.picked_up_at)).label('avg_delivery_time')
        ).filter(
            Delivery.created_at.between(start_date, end_date),
            Delivery.status == DeliveryStatus.DELIVERED,
        ...
        geographic_metrics = db.session.query(
            ...
            Delivery.created_at.between(start_date, end_date),
            Delivery.status == DeliveryStatus.DELIVERED
        ).group_by('lat_zone', 'lng_zone').all()
...
def handle_delivery_exception_task(self, delivery_id: int, exception_type: str, details: Dict[str, Any]):
    ...
    try:
        ...
        if exception_type == 'delay':
            ...
            breakdown_location = details.get('location', 'Unknown')
...
def reschedule_failed_delivery_task(self, delivery_id: int):
    ...
    try:
        ...
        delivery.estimated_delivery_time = next_day.replace(
            hour=delivery.estimated_delivery_time.hour,
            minute=delivery.estimated_delivery_time.minute
        )
...
def process_delivery_confirmation_task(self, delivery_id: int, confirmation_data: Dict[str, Any]):
    ...
    try:
        ...
        return {
            'success': True,
            'delivery_id': delivery_id,
            'completed_at': delivery.delivered_at.isoformat()
        }
...
def optimize_time_slots():
    ...
    try:
        ...
        time_slot_analysis = db.session.query(
            ...
            func.avg(func.extract('epoch', Delivery.actual_delivery_time - Delivery.scheduled_date)).label('avg_delivery_time'),
            ...
            Delivery.created_at.between(start_date, end_date)
        ).group_by(Delivery.scheduled_time_slot).all()
        ...
        for slot, total, avg_time, failed, rating in time_slot_analysis:
            if total > 0:
                ...
                needs_optimization = (
                    success_rate < 90 or  # Low success rate
                    avg_time_hours > 3 or  # Takes too long
                    (rating and rating < 4.0)  # Low customer satisfaction
                )
                ...
                optimization_results.append({
                    ...
                    'recommended_action': 'reduce_capacity' if avg_time_hours > 3 else 'maintain' if success_rate > 95 else 'review_process'
                })
