...
def notify_staff_new_order(self, order_id: int, order_info: dict = None):
    ...
    try:
        ...
        with app.app_context():
            ...
            delivery_persons = db.session.query(User.telegram_id).join(
                DeliveryPerson, DeliveryPerson.user_id == User.id
            ).filter(
                DeliveryPerson.notifications_muted == False,
...
def notify_staff_order_assigned(self, telegram_id: str, order_info: dict):
    ...
    _send_staff_webhook('/internal/order-assigned', {
...
def notify_staff_order_reassigned(
    self, old_telegram_id: str, new_telegram_id: str, order_info: dict
):
    """Notify both old and new delivery persons about a reassignment."""
    _send_staff_webhook('/internal/order-reassigned', {
...
def notify_staff_order_cancelled(self, telegram_id: str, order_info: dict):
    ...
    if not telegram_id:
        return
    _send_staff_webhook('/internal/order-cancelled', {
