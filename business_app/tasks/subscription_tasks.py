...
def update_subscription_analytics():
    ...
    try:
        ...
        mrr = (
            db.session.query(func.sum(Subscription.total_amount))
            .filter(Subscription.status == SubscriptionStatus.ACTIVE, Subscription.frequency == "monthly")
            .scalar()
            or 0
        )
        ...
        total_trials = (
            db.session.query(func.count(Subscription.id))
            .filter(Subscription.created_at.between(start_date, end_date), Subscription.trial_end_date.isnot(None))
            .scalar()
        )
...
def process_subscription_upgrades_downgrades():
    ...
    try:
        ...
        for subscription in subscriptions_with_changes:
            try:
                # TODO: SubscriptionPlan model does not exist; this whole task references
                # fields (subscription.plan, plan_id, pending_plan_change_id) that are not
                # on the Subscription model. Task is not registered with celery beat —
                # safe to delete or rewrite once plan-change semantics are decided.
                new_plan = SubscriptionPlan.query.get(subscription.pending_plan_change_id)  # noqa: F821
                if not new_plan:
                ...
                processed_count += 1
                logger.info(
                    f"Plan change processed for subscription {subscription.id}: {old_plan_name} -> {new_plan.get_translated('name', language)}"  # noqa: E501
                )
            ...
            try:
                ...
                if subscription.payment_token:
                    ...
                    subscription_service.cancel_subscription(
                        subscription.id, reason="Trial period expired without payment method", immediate=True
                    )
...
def send_subscription_usage_summary(self, subscription_id: int):
    ...
    try:
        ...
        usage_data = {
            ...
            "subscription_plan": (
                subscription.plan.get_translated("name", get_current_language()) if subscription.plan else "Standard"
            ),
        ...
        notification_service = NotificationService()
        notification_service.send_notification(
            subscription.user_id, "subscription_usage_summary", template_data=usage_data
        )
...
def _optimize_area_delivery_schedule(subscriptions: List[Subscription]) -> Dict[str, Any]:
    ...
    optimization["recommended_time_slots"] = [
        {"time_slot": slot, "subscription_count": count} for slot, count in sorted_slots[:5]  # Top 5 time slots
    ]
        ...
        for subscription in eligible_subscriptions:
            try:
                ...
                template_data = {
                    ...
                    "subscription_duration": (datetime.now(timezone.utc) - subscription.created_at).days
                    // 30,  # months
                }

                notification_service.send_notification(
                    subscription.user_id, "subscription_satisfaction_survey", template_data=template_data
                )
