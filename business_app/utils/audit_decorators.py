...
from .audit_logger import (
    audit_logger, AuditEventType, AuditSeverity,
    audit_login_success, audit_login_failure, audit_permission_denied,
    audit_suspicious_activity, audit_emergency_operation
)
from .exceptions import UnauthorizedError, ForbiddenError


def audit_api_call(event_type: AuditEventType = None, 
...
def audit_user_action(action_name: str, resource_type: str = "user_action",
                     severity: AuditSeverity = AuditSeverity.MEDIUM):
    """
    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            try:
                ...
                current_app.logger.warning(
                    f"ADMIN ACTION: {action_name} completed successfully by user {user_id} "
                    f"in {duration_ms}ms"
                )
                ...
                current_app.logger.error(
                    f"ADMIN ACTION: {action_name} failed for user {user_id} "
                    f"after {duration_ms}ms: {str(e)}"
                )
