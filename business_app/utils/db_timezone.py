...
from business_app.utils.timezone_utils import local_to_utc, parse_user_datetime
...
class TimezoneAwareQuery:
    ...
    @staticmethod
    def filter_by_date_range(query: Query, date_column, start_date=None, end_date=None, user_timezone=None) -> Query:
        """
        ...
        try:
            ...
            for day_num, day_name in enumerate(
                ...
                if open_time and close_time:
                    ...
                    time_filter = and_(
                        func.extract("hour", date_column) >= open_hour, func.extract("hour", date_column) <= close_hour
                    )
    ...
    def add_timezone_aware_aggregates(query: Query, date_column, user_timezone=None):
        ...
        try:
            ...
            aggregates = {
                ...
                "count_today": func.sum(case([(func.date(local_time) == func.current_date(), 1)], else_=0)),
                "count_this_week": func.sum(
                    case([(func.date_trunc("week", local_time) == func.date_trunc("week", func.now()), 1)], else_=0)
                ),
                "count_this_month": func.sum(
                    case([(func.date_trunc("month", local_time) == func.date_trunc("month", func.now()), 1)], else_=0)
                ),
            }
...
def validate_datetime_consistency(db_session):
    ...
    """
    validation_results = {"total_checks": 0, "passed_checks": 0, "failed_checks": 0, "issues": []}
    ...
    checks = [
        ...
        },
        {
            ...
            "query": "SELECT COUNT(*) FROM users WHERE last_login IS NOT NULL AND last_login AT TIME ZONE 'UTC' = last_login",  # noqa: E501
        },
        {
        ...
        try:
            ...
            if result == 0:
                ...
                validation_results["issues"].append(
                    {"check": check["name"], "issue": f"Found {result} records with timezone issues"}
                )
                logger.warning(f"FAIL: {check['name']} - {result} issues found")

        except Exception as e:
            validation_results["failed_checks"] += 1
            validation_results["issues"].append({"check": check["name"], "issue": f"Query failed: {str(e)}"})
