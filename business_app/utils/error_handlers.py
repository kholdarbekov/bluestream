...
class ExceptionMapper:
    ...
    def get_error_info(cls, exception: Exception) -> Tuple[int, str, str]:
        ...
        if isinstance(exception, WaterBusinessException):
            status_code, error_type = cls.EXCEPTION_MAPPING.get(type(exception), (500, "BUSINESS_ERROR"))
...
def sanitize_request_data(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for key, value in data.items():
        ...
        if any(sensitive in key_lower for sensitive in SENSITIVE_KEYS):
            ...
            sanitized[key] = [sanitize_request_data(item) if isinstance(item, dict) else item for item in value]
        else:
            sanitized[key] = value
...
def handle_api_exception(f):
    ...
    def decorated_function(*args, **kwargs):
        try:
            ...
            # Log the exception
            log_exception(exception=e, endpoint=f.__name__, user_id=user_id, request_data=request_data, level=log_level)
...
def handle_database_exceptions(f):
    ...
    def decorated_function(*args, **kwargs):
        try:
            ...
            from sqlalchemy.exc import IntegrityError, DataError, OperationalError, InvalidRequestError
            ...
            if isinstance(e, IntegrityError):
                if "unique constraint" in str(e).lower():
                    raise ConflictError("Resource already exists", details={"database_error": "UNIQUE_CONSTRAINT"})
                elif "foreign key constraint" in str(e).lower():
                    raise ValidationError(
                        "Invalid reference to related resource", details={"database_error": "FOREIGN_KEY_CONSTRAINT"}
                    )
                else:
                    raise ValidationError("Data integrity violation", details={"database_error": "INTEGRITY_ERROR"})

            elif isinstance(e, DataError):
                raise ValidationError("Invalid data format", details={"database_error": "DATA_ERROR"})

            elif isinstance(e, OperationalError):
                raise ExternalServiceError("Database connection error", details={"database_error": "CONNECTION_ERROR"})

            elif isinstance(e, InvalidRequestError):
                raise ConfigurationError("Database configuration error", details={"database_error": "INVALID_REQUEST"})
            ...
            try:
                ...
                if isinstance(e, requests.exceptions.Timeout):
                    raise ExternalServiceError(
                        f"{service_name} service timeout", details={"service": service_name, "error_type": "TIMEOUT"}
                    )
    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            if data:
                ...
                if errors:
                    raise ValidationError("Input validation failed", details={"validation_errors": errors})
...
def register_error_handlers(app):
    ...
    def handle_business_exception(error):
        ...
        return ErrorResponse.build_error_response(
            error_type=error_type, message=message, details=error.details, status_code=status_code
        )
...
def create_translated_error_response(
    error_code: ErrorCode, details: Optional[Dict[str, Any]] = None, **params: Any
) -> Tuple[Dict[str, Any], int]:
    """
    ...
    return ErrorResponse.build_error_response(
        error_type=error_type, message=message, details=details, status_code=status_code
    )
