...
def is_within_delivery_radius(customer_lat: float, customer_lon: float, 
                            store_lat: float, store_lon: float, 
                            radius_km: float = None) -> bool:
...
def calculate_delivery_fee(distance_km: float, order_total: float) -> int:
    """Calculate delivery fee based on distance and order total"""
    free_delivery_threshold = current_app.config['FREE_DELIVERY_THRESHOLD']
    ...
    base_fee = default_delivery_fee
    distance_fee = int(distance_km * 500)  # 500 UZS per km
