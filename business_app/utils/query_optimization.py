...
class EagerLoadingStrategy:
    ...
    def apply_strategy(cls, query: Query, strategy_name: str) -> Query:
        ...
        if not strategies and strategy_name in ['order_with_items', 'order_admin_detail', 'delivery_with_order']:
    ...
    def _build_runtime_strategy(cls, strategy_name: str, query: Query) -> List[Any]:
        """Build strategies at runtime based on query model"""
        strategies = []
        ...
        try:
            ...
            # Build safe loading strategies based on strategy name and model
            if strategy_name == 'order_with_items' and hasattr(model_class, 'user'):
                strategies.append(joinedload(model_class.user))
            elif strategy_name == 'delivery_with_order' and hasattr(model_class, 'order'):
...
class AggregationOptimizer:
    ...
    def get_user_statistics(user_ids: List[int]) -> Dict[int, Dict[str, Any]]:
        ...
        for stat in delivery_stats:
            stats[stat.user_id].update({
                'delivery_count': stat.delivery_count,
    ...
    def get_order_statistics(order_ids: List[int]) -> Dict[int, Dict[str, Any]]:
        ...
        for stat in payment_stats:
            stats[stat.order_id].update({
                ...
                'last_payment_date': stat.last_payment_date
            })
