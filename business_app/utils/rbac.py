...
class RBACValidator:
    ...
    def has_permission(self, user_role: UserRole, required_permission: Permission, user_id: int = None) -> bool:
        """
    ...
    def has_any_permission(
        self, user_role: UserRole, required_permissions: List[Permission], user_id: int = None
    ) -> bool:
        """
            ...
            try:
                ...
                if user_context["role"] != user_role:
                    current_app.logger.warning(
                        f"Role mismatch for user {user_id}: " f"JWT={user_role}, DB={user_context['role']}"
                    )
                ...
                if not has_access:
                    ...
                    current_app.logger.warning(
                        f"Access denied for user {user_id} ({user_role.value}) " f"to permission(s): {perm_names}"
                    )
    ...
    def decorator(f: Callable) -> Callable:
        ...
        def decorated_function(*args, **kwargs):
            ...
            try:
                ...
                current_app.logger.info(
                    f"AUDIT: User {user_id} successfully completed {operation} " f"in {elapsed_time:.3f}s"
                )
                ...
                elapsed_time = time.time() - start_time
                current_app.logger.warning(
                    f"AUDIT: User {user_id} failed {operation} " f"after {elapsed_time:.3f}s: {str(e)}"
                )
                raise
