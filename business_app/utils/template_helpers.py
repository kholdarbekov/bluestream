...
def get_language_label(language_code):
    ...
    language_labels = {"en": "English", "uz": "O'zbek", "ru": "Русский"}
...
def get_translation_completeness(entity, language=None):
    ...
    return {
        ...
        "missing_fields": [
            field for field in entity._translatable_fields if not is_field_translated(entity, field, language)
        ],
    }
...
def render_multilingual_text(entity, field_name, language=None, fallback=True, show_language_badge=False):
    ...
    if show_language_badge:
        ...
        badge = f'<span class="language-badge language-{language}" title="Content in {lang_label}">{language.upper()}</span> '  # noqa: E501
