...
class TranslationService:
    ...
    def _get_db_translation(self, key: str, language: str) -> Optional[str]:
        ...
        try:
            ...
            translation = Translation.query.filter_by(key=key, language=language, is_active=True).first()
    ...
    def set_translation(
        self, key: str, language: str, value: str, category: str = "general", description: str = None
    ) -> bool:
        """
        ...
        try:
            ...
            if translation:
                ...
                translation = Translation(
                    key=key, language=language, value=value, category=category, description=description, is_active=True
                )
    ...
    def get_all_translations(self, language: str) -> Dict[str, str]:
        ...
        try:
            translations = Translation.query.filter_by(language=language, is_active=True).all()
    ...
    def import_translations(self, translations_data: Dict[str, Dict[str, str]], category: str = "general") -> bool:
        """
            ...
            for language, translations in translations_data.items():
                for key, value in translations.items():
                    ...
                    if translation:
                        ...
                        translation = Translation(
                            key=key, language=language, value=value, category=category, is_active=True
                        )
                        db.session.add(translation)
    ...
    def warm_cache_for_category(self, category: str, languages: list = None) -> dict:
        ...
        try:
            ...
            translations = Translation.query.filter_by(category=category, is_active=True).all()
...
def set_translation(key: str, language: str, value: str, category: str = "general", description: str = None) -> bool:
    """Set translation in database"""
    return translation_service.set_translation(key, language, value, category, description)


def import_translations_from_dict(translations_data: Dict[str, Dict[str, str]], category: str = "general") -> bool:
    """Import translations from dictionary"""
...
def get_plural_rules(language: str) -> Dict[str, Any]:
    ...
    """
    rules = {
        "uz": {"forms": ["singular", "plural"], "rule": lambda n: 0 if n == 1 else 1},
        "en": {"forms": ["singular", "plural"], "rule": lambda n: 0 if n == 1 else 1},
        "ru": {
            "forms": ["one", "few", "other"],
            "rule": lambda n: (
                0
                if (n % 10 == 1 and n % 100 != 11)
                else 1 if (n % 10 in [2, 3, 4] and n % 100 not in [12, 13, 14]) else 2
            ),
        },
    }
...
def ngettext(singular_key: str, plural_key: str, count: int, language: str = None, **kwargs) -> str:
    """
