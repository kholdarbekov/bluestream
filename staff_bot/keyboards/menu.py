...
class MenuKeyboards:
    ...
    def main_menu(language: str, staff_roles: List[str] = None) -> ReplyKeyboardMarkup:
        ...
        if is_driver:
            ...
            keyboard.append([
                f"🧪 {i18n.get('staff.menu.tryouts', language)}",
                f"💰 {i18n.get('staff.menu.cash', language)}",
            ])
        elif is_operator:
            # Operator-only users still need the New Orders entry point
            keyboard.append([
                f"📦 {i18n.get('staff.menu.new_orders', language)}",
                f"👤 {i18n.get('staff.menu.create_client', language)}",
            ])
        ...
        if is_operator and is_driver:
            ...
            keyboard.append([
                f"📦 {i18n.get('staff.menu.create_order', language)}",
            ])
        elif is_operator:
            keyboard.append([
                f"📦 {i18n.get('staff.menu.create_order', language)}",
    ...
    def tryouts_hub(language: str) -> InlineKeyboardMarkup:
        ...
        return InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton(
                f"🧪 {i18n.get('staff.menu.tryout_tasks', language)}",
    ...
    def cash_hub(language: str) -> InlineKeyboardMarkup:
        ...
        return InlineKeyboardMarkup([
            [InlineKeyboardButton(
                f"🧾 {i18n.get('staff.menu.cash_reconciliation', language)}",
                callback_data="staff_reconcile_session",
            )],
            [InlineKeyboardButton(
                f"💸 {i18n.get('staff.menu.collect_cod_debt', language)}",
                callback_data="staff_cod_collect_menu",
            )],
            [InlineKeyboardButton(
                f"📦 {i18n.get('staff.menu.bottle_collection', language)}",
            ...
            [InlineKeyboardButton(
                f"📊 {i18n.get('staff.menu.my_bottle_accountability', language)}",
    ...
    def profile_hub(language: str, staff_roles: List[str] = None) -> InlineKeyboardMarkup:
        ...
        if is_driver:
            rows.append([InlineKeyboardButton(
                f"📊 {i18n.get('staff.profile.view_stats', language)}",
                callback_data="staff_my_stats",
            )])
            rows.append([InlineKeyboardButton(
                f"📋 {i18n.get('staff.profile.view_history', language)}",
